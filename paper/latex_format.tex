...
There wasn't much cleaning needed for our dataset apart from converting our categorical columns (Credentials & Specialty) to values. We used the label encoder provided by sklearn.preprocessing library to encode the columns with values between 0 and n_classes - 1.
...
\begin{enumerate}
   ...
   \item We also found that the dataset has a slightly higher number of opioid prescribers, when we plot the opioid prescriber count vs non opioid prescriber count.
 \end{enumerate}
